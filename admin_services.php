...
if (isset($_POST['mark_success'])) {
  ...
  try {
      ...
      // Set success message
      $success_msg = "Application marked as successful!";
  } catch (Exception $e) {
      // Set error message
      $error_msg = "Error updating status: " . $e->getMessage();
  }
...
try {
  ...
  if (!empty($sql_params)) {
      ...
      $stmt->execute();
      $result = $stmt->get_result();
      // Don't close the statement here, we'll close it after we're done with it
  } else {
  ...
  // Now close the statement if it exists
  if ($stmt !== null) {
      $stmt->close();
  }
} catch (Exception $e) {
